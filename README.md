...
>The list of figures feature requires that a distinctive character be inserted in the text; this tells the PDF scanner that there is a figure present. The first step in generating a list of figures is therefore to replace the beginning of each label with the UTF-8 character 'REVERSED ROTATED FLORAL HEART BULLET', which looks like this: ☙. This symbol was chosen for its obscurity, however, in the rare event that your document contains this symbol, you will need to replace it with something else before generating the list of figures. You can use Docs' search and replace function to replace it, then you can use that same function to restore it after generating the list of figures.
